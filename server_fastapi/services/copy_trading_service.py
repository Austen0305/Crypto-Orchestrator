...
class CopyTradingService:
    ...
    async def follow_trader(
        ...
        try:
            ...
            existing_stmt = select(Follow).where(
                and_(Follow.follower_id == follower_id, Follow.trader_id == trader_id)
            )
    ...
    async def get_followed_traders(self, follower_id: int) -> List[Dict]:
        ...
        try:
            stmt = (
                select(Follow, User)
                .join(User, Follow.trader_id == User.id)
                .where(
                    and_(Follow.follower_id == follower_id, Follow.is_active == True)
                )
            )
            result = await self.db.execute(stmt)
            follows = result.all()
    ...
    async def copy_trade(
        ...
        try:
            ...
            trade_stmt = select(Trade).where(
                and_(Trade.id == original_trade_id, Trade.user_id == trader_id)
            )
            ...
            try:
                ...
                if not validation["valid"]:
                    ...
                    await self.db.commit()
                    raise ValueError(
                        f"Trade validation failed: {copied_trade_record.error_message}"
                    )
                ...
                if executed_trade and "id" in executed_trade:
                    ...
                    copied_trade_record.copied_price = executed_trade.get(
                        "price", original_trade.price
                    )
                    ...
                    copied_trade_record.status = "failed"
                    copied_trade_record.error_message = (
                        "Trade execution returned no result"
                    )
                    await self.db.commit()
        ...
        try:
            ...
            for copied_trade in copied_trades:
                if copied_trade.copied_trade_id:
                    ...
                    trade_stmt = select(Trade).where(
                        Trade.id == copied_trade.copied_trade_id
                    )
                    trade_result = await self.db.execute(trade_stmt)
