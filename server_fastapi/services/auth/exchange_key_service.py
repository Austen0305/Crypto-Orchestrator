...
class ExchangeKeyService:
    ...
    def __init__(self):
        ...
        if not encryption_key:
            ...
            logger.warning(
                "EXCHANGE_KEY_ENCRYPTION_KEY not set, using default (NOT SECURE FOR PRODUCTION)"
            )
    ...
    async def create_api_key(
        ...
        try:
            ...
            if ExchangeAPIKey is None:
                ...
                logger.info(
                    f"Created exchange API key for user {user_id_int} on {exchange} (in-memory)"
                )
                return api_key_obj
            ...
            async with get_db_context() as db:
                ...
                logger.info(
                    f"Created exchange API key for user {user_id_int} on {exchange}"
                )
                return {
        ...
        try:
            ...
            if ExchangeAPIKey is None:
                ...
                for key_obj in self._memory_store.values():
                    if (
                        key_obj.get("user_id") == user_id_int
                        and key_obj.get("exchange") == exchange
                        and key_obj.get("is_active", True)
                    ):
                        response = {
                            "id": key_obj["id"],
                            ...
                            "created_at": (
                                key_obj.get("created_at").isoformat()
                                if isinstance(key_obj.get("created_at"), datetime)
                                else None
                            ),
                        }
                        if include_secrets:
                            response["api_key"] = self._decrypt(
                                key_obj["api_key_encrypted"]
                            )
                            response["api_secret"] = self._decrypt(
                                key_obj["api_secret_encrypted"]
                            )
                            if key_obj.get("passphrase_encrypted"):
                                response["passphrase"] = self._decrypt(
            ...
            async with get_db_context() as db:
                ...
                if include_secrets:
                    ...
                    response["api_secret"] = self._decrypt(
                        api_key_obj.api_secret_encrypted
                    )
                    if api_key_obj.passphrase_encrypted:
                        response["passphrase"] = self._decrypt(
                            api_key_obj.passphrase_encrypted
                        )
    ...
    async def validate_api_key(self, user_id: str, exchange: str) -> bool:
        ...
        try:
            api_key_data = await self.get_api_key(
        ...
        try:
            ...
            if ExchangeAPIKey is None:
                ...
                if hasattr(self, "_memory_store"):
                    keys_to_delete = [
                        key_id
                        for key_id, key_obj in self._memory_store.items()
                        if (
                            key_obj.get("user_id") == user_id_int
                            and key_obj.get("exchange") == exchange
                        )
                    ...
                    logger.info(
                        f"Deleted exchange API key for user {user_id_int} on {exchange} (in-memory)"
                    )
            ...
            logger.info(
                f"Deleted exchange API key for user {user_id_int} on {exchange}"
            )
            return True
                ...
                return [
                    {
                        ...
                        "is_validated": key_obj.get("is_validated", False),
                        "validated_at": (
                            key_obj.get("validated_at").isoformat()
                            if isinstance(key_obj.get("validated_at"), datetime)
                            else None
                        ),
                        "last_used_at": (
                            key_obj.get("last_used_at").isoformat()
                            if isinstance(key_obj.get("last_used_at"), datetime)
                            else None
                        ),
                        "created_at": (
                            key_obj.get("created_at").isoformat()
                    ...
                    for key_obj in self._memory_store.values()
                    if key_obj.get("user_id") == user_id_int
                    and key_obj.get("is_active", True)
                ]
                ...
                return [
                    {
                        ...
                        "validated_at": (
                            api_key.validated_at.isoformat()
                            if api_key.validated_at
                            else None
                        ),
                        "last_used_at": (
