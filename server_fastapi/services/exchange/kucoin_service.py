...
class KuCoinService:
    ...
    def __init__(
        ...
        if use_mock is not None:
            ...
            self.use_mock = (
                ...
                or os.getenv("USE_MOCK_KUCOIN", "false").lower() == "true"
            )
    ...
    async def get_fees(self) -> KuCoinFee:
        ...
        try:
            ...
            base_taker = 0.001
            maker_fee = max(
                0.0001, base_maker * (1 - vip_level * 0.05)
            )  # 5% reduction per VIP level
            taker_fee = max(0.0001, base_taker * (1 - vip_level * 0.05))

            return KuCoinFee(maker=maker_fee, taker=taker_fee, vip_level=vip_level)
        except Exception as error:
