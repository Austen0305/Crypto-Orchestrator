...
class SmartRoutingService:
    ...
    async def get_best_price(
        ...
        try:
            ...
            quotes = await self._get_quotes_from_exchanges(
            ...
            quotes_sorted = sorted(
                quotes, key=lambda q: q.effective_price, reverse=reverse
            )
            ...
            if side == "buy":
                savings = (
                    worst_quote.effective_price - best_quote.effective_price
                ) * amount
            else:
                savings = (
                    best_quote.effective_price - worst_quote.effective_price
                ) * amount
    ...
    async def get_slippage_aware_route(
        ...
        try:
            ...
            filtered_quotes = [
                q for q in quotes if q.estimated_slippage <= max_slippage
            ]
            ...
            reverse = side == "sell"
            quotes_sorted = sorted(
                filtered_quotes, key=lambda q: q.effective_price, reverse=reverse
            )
        ...
        try:
            ...
            for quote in quotes:
                ...
                if max_price != min_price:
                    price_score = (quote.effective_price - min_price) / (
                        max_price - min_price
                    )
                else:
                    price_score = 0.5
                ...
                slippage_score = (
                    quote.estimated_slippage / max_slippage if max_slippage > 0 else 0.5
                )
    ...
    async def _get_quotes_from_exchanges(
        self, symbol: str, side: str, amount: float, exchanges: List[str]
    ) -> List[OrderQuote]:
            ...
            try:
                ...
                if side == "buy":
                    ...
                    estimated_slippage = self._estimate_slippage(
                        orderbook["asks"], amount, side
                    )
                else:
                    price = ticker.get("bid", ticker.get("last", 0))
                    # Estimate slippage from order book
                    estimated_slippage = self._estimate_slippage(
                        orderbook["bids"], amount, side
                    )
    ...
    def _estimate_slippage(
        self, orderbook_side: List[List[float]], amount: float, side: str
    ) -> float:
        ...
        **kwargs,
        ...
        try:
            if strategy == RoutingStrategy.BEST_PRICE:
                ...
                result = await self.get_fee_optimized_route(
                    symbol, side, amount, exchanges
                )
            elif strategy == RoutingStrategy.SLIPPAGE_AWARE:
                max_slippage = kwargs.get("max_slippage", 0.005)
                result = await self.get_slippage_aware_route(
                    symbol, side, amount, exchanges, max_slippage
                )
            else:  # COMBINED
