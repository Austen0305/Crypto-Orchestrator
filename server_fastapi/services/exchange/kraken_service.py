...
class KrakenService:
    ...
    def __init__(
        ...
        if use_mock is not None:
            ...
            self.use_mock = (
                ...
                or os.getenv("USE_MOCK_KRAKEN", "false").lower() == "true"
            )
        ...
        if not self.use_mock:
            try:
                ...
                if not exchange_api_key or not exchange_api_secret:
                    logger.warning(
                        f"No API keys provided for Kraken. Exchange will be read-only or unavailable."
                    )
    ...
    async def get_historical_data(
        ...
        try:
            ...
            logger.error(
                f"Failed to fetch historical data for {symbol} on kraken: {error}"
            )
            return []
    ...
    async def get_all_trading_pairs(self) -> List[TradingPair]:
        ...
        try:
            ...
            if not self.exchange:
                logger.warning(
                    "Exchange client missing or does not support fetchTickers"
                )
                return []
    ...
    async def get_order_book(self, pair: str) -> Dict[str, List[List[float]]]:
        try:
            ...
            return {"bids": order_book["bids"][:10], "asks": order_book["asks"][:10]}
        except Exception as error:
        ...
        try:
            ...
            if type_ == "market":
                ...
                if side == "buy":
                    ...
                    order = await self.exchange.createLimitSellOrder(
                        pair, amount, price
                    )
            else:
        ...
        try:
            if self.use_mock:
                ...
                close = (
                    ...
                    else 3500 if pair.startswith("ETH") else 1
                )
                open_ = close - 10
    ...
    async def get_api_quota(self) -> Dict[str, Any]:
        try:
            ...
            if hasattr(self.exchange, "rateLimit") and isinstance(
                self.exchange.rateLimit, (int, float)
            ):
                return {
    ...
    def calculate_fee(
        self, amount: float, price: float, is_maker: bool = False, volume_usd: float = 0
    ) -> float:
