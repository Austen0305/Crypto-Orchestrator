...
class WalletService:
    ...
    async def get_or_create_wallet(
        self, user_id: int, currency: str = "USD", wallet_type: str = "trading"
    ) -> Wallet:
        ...
        try:
            ...
            if not wallet:
                ...
                logger.info(
                    f"Created wallet for user {user_id}: {currency} {wallet_type}"
                )
    ...
    async def deposit(
        ...
        try:
            ...
            if not is_valid:
                ...
                logger.warning(
                    f"Deposit validation failed for user {user_id}: {error_message}"
                )
                raise ValueError(f"Deposit validation failed: {error_message}")
        except ImportError:
            # Safety service not available, proceed with basic checks
            logger.warning(
                "Deposit safety service not available, using basic validation"
            )
        except ValueError:
            raise
        except Exception as e:
        ...
        try:
            ...
            if payment_intent_id:
                try:
                    ...
                    success, transaction_id, error_msg = (
                    ...
                    if not success:
                        raise ValueError(
                            error_msg or "Failed to process deposit safely"
                        )
                    ...
                    logger.warning(
                        "Deposit safety service not available, using basic processing"
                    )
            ...
            transaction = WalletTransaction(
                ...
                or f"Deposit {amount} {currency} (Fee: {deposit_fee:.2f} {currency})",
            ...
            logger.info(
                f"Deposit initiated: user {user_id}, amount {amount} {currency}"
            )
    ...
    async def confirm_deposit(
        self, transaction_id: int, payment_intent_id: str
    ) -> bool:
        """
        ...
        try:
            ...
            if not transaction:
                logger.warning(
                    f"Transaction not found: {transaction_id}, payment_intent: {payment_intent_id}"
                )
                return False
            ...
            is_verified, error_msg, payment_details = (
                await deposit_safety_service.verify_payment_received(
                    payment_intent_id, amount_decimal, transaction.currency
                )
            )
            ...
            logger.warning(
                "Deposit safety service not available, using basic confirmation"
            )
    ...
    async def withdraw(
        ...
        try:
            ...
            is_valid, errors, metadata = (
                await real_money_safety_service.validate_real_money_withdrawal(
                    user_id=user_id,
                    amount=amount_decimal,
                    currency=currency,
            ...
            if not is_valid:
                error_message = "; ".join(errors)
                logger.warning(
                    f"Withdrawal validation failed for user {user_id}: {error_message}"
                )
            ...
            if destination and not skip_whitelist_check:
                try:
                    from .security.withdrawal_whitelist_service import (
                        withdrawal_whitelist_service,
                    )

                    is_whitelisted = (
                        await withdrawal_whitelist_service.is_address_whitelisted(
                            user_id=user_id,
                            address=destination,
                            currency=currency,
                            db=self.db,
                    ...
                    logger.warning(
                        "Withdrawal whitelist service not available, skipping check"
                    )
                except ValueError:
                    raise
                except Exception as e:
                    logger.warning(
                        f"Whitelist check failed: {e}, proceeding with withdrawal"
                    )
            ...
            try:
                ...
                logger.warning(
                    f"Fraud detection check failed: {e}, proceeding with withdrawal"
                )
        except ImportError:
            # Safety service not available, proceed with basic checks
            logger.warning(
                "Real money safety service not available, using basic validation"
            )
        except Exception as e:
            logger.error(f"Safety validation error: {e}", exc_info=True)
            raise
        ...
        try:
            ...
            logger.info(
                f"Withdrawal initiated: user {user_id}, amount {amount} {currency}"
            )
    ...
    async def confirm_withdrawal(self, transaction_id: int) -> bool:
    ...
    async def get_wallet_balance(self, user_id: int, currency: str = "USD") -> Dict:
            ...
            return [
                {
                    ...
                    "processed_at": (
                        t.processed_at.isoformat() if t.processed_at else None
                    ),
                }
