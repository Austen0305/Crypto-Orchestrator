...
from ..services.backtesting.strategy_optimizer import (
    StrategyOptimizer,
    OptimizationConfig,
    ParameterRange,
)
...
async def run_backtest(
    ...
    try:
        ...
        if not request.historicalData or len(request.historicalData) < 100:
            raise HTTPException(
                status_code=400,
                detail="Insufficient historical data (minimum 100 data points required)",
            )
        ...
        logger.info(
            f"Backtest completed for bot {request.botId}: {result.totalTrades} trades, {result.totalReturn:.2%} return"
        )
        return summary
...
async def reset_paper_trading(
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(
                status_code=500, detail="Failed to reset paper trading account"
            )
...
async def run_strategy_optimization(
    ...
    try:
        ...
        results = await optimizer.optimize_strategy(
            config, historical_data, request.botId
        )
        ...
        raise HTTPException(
            status_code=500, detail="Failed to run strategy optimization"
        )
...
async def save_backtest_result(request: SaveBacktestRequest) -> BacktestSummary:
    ...
    try:
        ...
        logger.info(
            f"Backtest result saved: {result_id} for bot {request.result.botId}"
        )
        return saved_result
