...
async def get_binance_orderbook(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch order book: {str(e)}"
        )


@router.get("/binance/fees", response_model=Dict)
async def get_binance_fees(current_user: dict = Depends(get_current_user)):
...
async def get_binance_trading_pairs(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch trading pairs: {str(e)}"
        )
...
async def get_coinbase_fees(current_user: dict = Depends(get_current_user)):
    """Get Coinbase fee structure"""
    try:
        fees = await coinbase_service.get_fees()
        return {"maker": fees.maker, "taker": fees.taker, "tier": fees.tier}
    except Exception as e:
...
async def get_coinbase_account_info(current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch account info: {str(e)}"
        )
...
async def get_kucoin_fees(current_user: dict = Depends(get_current_user)):
    """Get KuCoin fee structure with VIP level"""
    try:
        fees = await kucoin_service.get_fees()
        return {"maker": fees.maker, "taker": fees.taker, "vip_level": fees.vip_level}
    except Exception as e:
...
class RoutingRequest(BaseModel):
    ...
    amount: float
    strategy: str = (
        "combined"  # 'best_price', 'fee_optimized', 'slippage_aware', 'combined'
    )
...
async def get_smart_routing_quote(
    request: RoutingRequest, current_user: dict = Depends(get_current_user)
):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get routing quote: {str(e)}"
        )
    ...
    try:
        ...
        if strategy == RoutingStrategy.BEST_PRICE:
            result = await smart_routing_service.get_best_price(
                request.symbol, request.side, request.amount, request.exchanges
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to compare prices: {str(e)}"
        )
