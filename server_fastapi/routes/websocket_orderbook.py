...
async def websocket_orderbook(
    websocket: WebSocket, pair: str, token: Optional[str] = None
):
    """
    ...
    subscription_id = await orderbook_streaming_service.subscribe(
        pair, lambda ob: orderbook_callback(pair, ob), update_interval=1.0
    )
    ...
    try:
        ...
        await orderbook_streaming_service.unsubscribe(
            pair, lambda ob: orderbook_callback(pair, ob)
        )
        logger.info(f"Order book WebSocket disconnected for {pair}")
    except Exception as e:
