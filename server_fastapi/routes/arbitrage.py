...
class ArbitrageType(str, Enum):
    ...
    CROSS_EXCHANGE_TRIANGULAR = (
        "cross_exchange_triangular"  # Triangular across exchanges
    )
...
class ArbitrageConfig(BaseModel):
    ...
    max_position_size_usd: float = Field(
        1000.0, description="Max position size per trade"
    )
...
class ArbitrageScanner:
    ...
    async def fetch_exchange_prices(
        self, exchange: str, symbols: List[str], user_id: Optional[int] = None
        ...
        try:
            ...
            if api_key_data and api_key_data.get("is_validated"):
                ...
                exchange_service = ExchangeService(
                    name=exchange, use_mock=False  # Force real mode
                )
            ...
            for symbol in symbols:
                try:
                    ...
                    try:
                        ...
                        fee_percent = (
                            (fees.taker * 100) if hasattr(fees, "taker") else 0.1
                        )
                    except:
                        pass
                    ...
                    logger.warning(
                        f"Failed to fetch price for {symbol} on {exchange}: {e}"
                    )
                    continue
            ...
            if settings.production_mode or settings.is_production:
                ...
                logger.warning(
                    f"Using mock data fallback for {exchange} in development"
                )
                for symbol in symbols:
                    base_price = 50000.0
    ...
    async def scan_simple_arbitrage(
        self, symbols: List[str], exchanges: List[str], config: ArbitrageConfig
        ...
        fetch_tasks = [
            self.fetch_exchange_prices(
                exchange, symbols, user_id=None
        ...
        for symbol in symbols:
            ...
            best_sell_exchange = max(exchanges, key=lambda e: all_prices[e][symbol].bid)
            best_sell_price = all_prices[best_sell_exchange][symbol].bid

            # Find best ask (lowest price to buy at)
            best_buy_exchange = min(exchanges, key=lambda e: all_prices[e][symbol].ask)
            ...
            confidence = self._calculate_confidence(
                spread_percent, profit_percent, max_latency, min_volume
            )
    ...
    async def execute_arbitrage(
        self, opportunity: ArbitrageOpportunity, position_size_usd: float
    ) -> ArbitrageExecution:
        ...
        try:
            ...
            actual_profit = (
                sell_result["executed_value"]
                - buy_result["executed_value"]
                - buy_result["fee"]
                - sell_result["fee"]
            )
            ...
            slippage_percent = (
                (expected_profit - actual_profit) / expected_profit
            ) * 100
    ...
    async def _place_order(
        self, exchange: str, symbol: str, side: str, price: float, size_usd: float
    ) -> Dict:
...
async def arbitrage_scan_loop(config: ArbitrageConfig):
    ...
    while scanner_running:
        try:
            ...
            for opp in opportunities:
                ...
                if config.auto_execute and opp.execution_confidence > 80:
                    execution = await scanner.execute_arbitrage(
                        opp, config.max_position_size_usd
                    )
            ...
            expired = [
                opp_id
                for opp_id, opp in active_opportunities.items()
...
@router.post("/start")
async def start_scanner(config: ArbitrageConfig, background_tasks: BackgroundTasks):
    """
    ...
    logger.info(
        f"Started arbitrage scanner with {len(config.enabled_exchanges)} exchanges"
    )

    return {"success": True, "message": "Arbitrage scanner started", "config": config}
...
async def get_opportunities(
    min_profit: Optional[float] = None, symbol: Optional[str] = None, limit: int = 50
):
    """
...
async def execute_opportunity(opportunity_id: str, position_size_usd: float = 1000.0):
    """
...
async def get_arbitrage_stats():
    ...
    success_rate = (
        (len(successful) / len(execution_history) * 100) if execution_history else 0
    )
    ...
    best_exec = (
        max(successful, key=lambda e: e.actual_profit_usd) if successful else None
    )
