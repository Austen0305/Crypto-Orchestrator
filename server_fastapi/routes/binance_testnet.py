...
from server_fastapi.services.exchange.binance_testnet_service import get_binance_testnet_service
...
async def get_balance():
    ...
    if not result['success']:
        raise HTTPException(status_code=500, detail=result.get('error', 'Balance fetch failed'))
...
async def get_order_history(
    ...
    if not result['success']:
        raise HTTPException(status_code=500, detail=result.get('error', 'Order history fetch failed'))
...
async def get_open_orders(
    ...
    if not result['success']:
        raise HTTPException(status_code=500, detail=result.get('error', 'Open orders fetch failed'))
...
async def get_ticker(symbol: str):
    ...
    if not result['success']:
        raise HTTPException(status_code=500, detail=result.get('error', 'Ticker fetch failed'))
