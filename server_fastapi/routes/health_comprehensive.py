    ...
    try:
        ...
        return DependencyHealth(
            ...
            details={
                ...
                "available": (
                    pool_size - checked_out if pool_size and checked_out else None
                ),
            },
        )
...
async def detailed_health_check(
    db: AsyncSession = Depends(get_db_session), include_optional: bool = False
) -> HealthResponse:
    """
...
async def check_specific_dependency(
    dependency_name: str, db: AsyncSession = Depends(get_db_session)
) -> DependencyHealth:
