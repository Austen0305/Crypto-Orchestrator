...
class TrainModelRequest(BaseModel):
    ...
    validation_split: float = Field(0.2, ge=0.1, le=0.3, description="Validation split ratio")


class PredictionRequest(BaseModel):
    """Prediction request"""
    recent_data: List[List[float]] = Field(..., description="Recent price data for prediction")
...
async def predict(request: PredictionRequest):
    ...
    if not service.ml_available:
        raise HTTPException(
            status_code=503,
            detail="ML libraries not available"
        )
...
async def health_check():
    ...
    return {
        ...
        'message': 'ML service operational' if status['ml_available'] else 'ML libraries not installed'
    }
